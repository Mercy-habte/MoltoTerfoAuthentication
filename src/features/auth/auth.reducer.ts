...
const getResponseData=(value)=> value.data.user
...
export const login = (usr:UserModel, history, checked:boolean): AppThunk => async dispatch => {
    try {
        ...
        // const usrRes = await axios.post(, JSON.stringify(usr));


        dispatch(LOG_g("data", usrRes.data.toString()))
        console.log("usr res-----",usrRes.data)
        ...
        if (user.role === "jobseeker") history.push(Routes.JOBSEEKER)
        else
            history.push("/");
        return
